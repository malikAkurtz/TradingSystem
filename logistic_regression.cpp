...
int main() {
    int feature_col = 1;
    std::vector<std::vector<float>> dataMatrix = parseCSV("data-2.csv");
    std::vector<std::vector<float>> dataMatrix_T = takeTranspose(dataMatrix);
    std::vector<float> labels = dataMatrix_T[feature_col];
    std::vector<std::vector<float>> feature_matrix;

    for (int i = 0; i < dataMatrix_T.size(); i++) {
        if (i != feature_col) {
            feature_matrix.push_back(dataMatrix_T[i]);
        }
    }



    LogisticRegression LR(0.0001);
    LR.fit(feature_matrix, labels);
    

    std::vector<bool> predictions = LR.getPredictions(feature_matrix);

    for (int i = 0; i < labels.size(); i++) {
        std::cout << "Predicted vs Actual: " << predictions[i] << "," << labels[i] << std::endl;
    }
