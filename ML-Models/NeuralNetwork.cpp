...
class NeuralNetwork {
    ...
    NeuralNetwork(float learningrate, int num_epochs, LossFunction lossFunction, int batchSize) : inputLayer(nullptr){
    ...
    //     int num_batches = static_cast<int>(ceil(static_cast<double>(num_samples) / this->batch_size));

    //     std::vector<std::vector<std::vector<double>>> batches_of_features = createBatches(featuresMatrix, num_batches);

    //     std::vector<std::vector<std::vector<double>>> batches_of_labels = createBatches(labels, num_batches);
    ...
    //             std::vector<std::vector<std::vector<double>>> X(batches_of_features[i].size());
    //             // filling up X to be a vector of column vectors representing samples that
    //             // we will pass through the network from left to right
    //             for (int s = 0; s < X.size(); s++) {
    //                 X[s] = vector1DtoColumnVector(batches_of_features[i][s]);
    //             }

    //             // filling up Y to be a vector of column vectors representing labels
    //             std::vector<std::vector<std::vector<double>>> Y(batches_of_labels[i].size());
    //             for (int s = 0; s < Y.size(); s++) {
    //                 Y[s] = vector1DtoColumnVector(batches_of_features[i][s]);
    //             }
    ...
    // this will return a vector of vectors of column vectors representing the outputs of the NN
    // for each sample that is passed in through features Matrix
    std::vector<std::vector<std::vector<std::vector<double>>>> getPredictions(std::vector<std::vector<double>> featuresMatrix) {

        int num_samples = featuresMatrix.size();

        std::vector<std::vector<std::vector<std::vector<double>>>> predictions;
